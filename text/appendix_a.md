...
В Python для игровой графики обычно используют [Pygame](https://www.pygame.org/), а в TypeScript --- Canvas API, PixiJS или Phaser. Raylib-ocaml --- это привязки к C-библиотеке Raylib, которая существует для множества языков (C, Python, Rust, Go и др.). Подход идентичен: игровой цикл `while` с обработкой ввода, обновлением состояния и отрисовкой. Если вы знакомы с Pygame (`pygame.event.get()`, `screen.blit()`, `pygame.display.flip()`), API raylib покажется знакомым.
```
...
```admonish tip title="Для Python/TS-разработчиков"
В Pygame или HTML Canvas состояние обычно хранится в глобальных переменных или объектах класса. В OCaml `ref` --- это аналог мутабельной переменной, но в функциональном подходе мы стремимся отделить чистую логику от побочных эффектов. Функция `update` принимает состояние и возвращает новое --- как в Redux (TypeScript) или в архитектуре Elm. Это делает логику тестируемой без запуска графики.
```
...
Подробнее о работе с внешними C-библиотеками (что используется в raylib-ocaml) --- в главе [Foreign Function Interface](https://dev.realworldocaml.org/foreign-function-interface.html) книги Real World OCaml.
```

## Структура проекта упражнений

Откройте директорию `exercises/appendix_a`:
