...
В этой главе мы разберём **модульную систему** OCaml. Вместо классов типов Haskell OCaml использует модули для абстракции, инкапсуляции и параметрического полиморфизма.
...
Представьте, что вы пишете библиотеку для работы с HTTP-клиентом или базой данных. В рантайме IO может быть разным: Lwt (асинхронный через промисы), Eio (эффект-ориентированный) или обычный синхронный stdlib. Привязка к одной реализации лишает пользователей выбора и делает библиотеку непригодной для других рантаймов.

### ООП-аналогия: Dependency Inversion Principle

Если вы знакомы с принципами SOLID, это — **Dependency Inversion Principle (DIP)**: зависеть от абстракций, а не от конкретных реализаций.

В TypeScript решение выглядело бы так:

```typescript
// Абстракция: интерфейс IO
interface IO<A> {
  flatMap<B>(f: (a: A) => IO<B>): IO<B>;
}

// Библиотека зависит от интерфейса, а не от Promise
class Service {
  constructor(private io: IOFactory) {}
  process(data: string): IO<string> { /* ... */ }
}

// Клиент внедряет нужную реализацию
const syncService  = new Service(new SyncIOFactory());
const asyncService = new Service(new PromiseIOFactory());
```

В ООП зависимость внедряется **в рантайме** через конструктор или DI-фреймворк. В OCaml она внедряется **на этапе компиляции** через функторы — готовый модуль уже «зашит» в конкретный IO без накладных расходов.
...
Сигнатура `IO` — OCaml-аналог интерфейса `IO<A>`. Тип `'a t` — «обёрнутое» значение: промис в Lwt, эффект в Eio или само значение в синхронном случае. `return` поднимает значение в контекст, `bind` выстраивает цепочку операций (аналог `flatMap`/`then`).

### Функтор: compile-time Dependency Injection
...
`Make_service` — это «фабрика» модулей: принимает реализацию IO и возвращает готовый модуль сервиса. Логика сервиса не знает, какой IO будет использован — это определяется **при создании модуля**, а не при каждом вызове функции.

### Инстанцирование для разных рантаймов

```ocaml
(* Синхронный IO: 'a t = 'a, return = id, bind = apply *)
...
(* Асинхронный IO через Lwt *)
module Lwt_IO : IO with type 'a t = 'a Lwt.t = struct
  type 'a t = 'a Lwt.t
...
module Async_service = Make_service(Lwt_IO)

let result = Sync_service.process "hello"
(* result = "processed: HELLO" — обычный синхронный вызов *)

let promise = Async_service.process "hello"
(* promise : string Lwt.t — асинхронный вызов *)
```

`Sync_service` и `Async_service` — два разных модуля с одинаковым интерфейсом, но разной семантикой IO. `Sync_IO` — тождественная монада: `type 'a t = 'a`, `return = id`, `bind = apply`. После подстановки `Sync_IO` весь монадический код компилируется в обычные синхронные вызовы — никакой динамической диспетчеризации.

```admonish tip title="Для ООП-разработчиков"
Сравнение подходов:

| | ООП (TypeScript) | OCaml (функторы) |
|--|--|--|
| Абстракция | `interface IO<A>` | `module type IO` |
| Реализация | класс, реализующий интерфейс | модуль с сигнатурой `IO` |
| Внедрение | конструктор, DI-фреймворк | `Make_service(Lwt_IO)` |
| Момент привязки | рантайм | компиляция |
| Накладные расходы | виртуальная диспетчеризация | нет (специализация) |

Функтор — это буквально compile-time DI-контейнер: вместо того чтобы в рантайме выяснять, какой объект реализует интерфейс, компилятор специализирует код под конкретный модуль ещё при сборке.
```

Этот паттерн широко используется в экосистеме OCaml (Cohttp, Irmin, Mirage OS) для создания библиотек, одновременно совместимых с Lwt, Eio и синхронным IO.
...
- Модули — основа организации кода в OCaml.
- Сигнатуры и абстрактные типы — инкапсуляция реализации.
- Функторы — параметризованные модули, аналог generics на уровне модулей.
- Модули первого класса — передача модулей как значений.
- Полугруппы и моноиды — алгебраические абстракции через модули.
- `open` и `include` — композиция модулей.

Соглашения по организации модулей, именованию и стилю кода см. в [Приложении «Конвенции OCaml»](appendix_d.md).

Следующая глава — обработка ошибок: типы `option` и `result`, let-операторы и паттерн накопления ошибок.
