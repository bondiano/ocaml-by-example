...
В этой главе — идиоматическая обработка ошибок в OCaml.
...
- **Накопление ошибок** — паттерн аппликативной валидации.
...
Цепочка читается сверху вниз: берём адрес → из адреса берём индекс → форматируем строку → если в любом шаге встретился `None`, `Option.value` подставляет дефолт и возвращает `string` (не `string option`). `Option.map` (в отличие от `Option.bind`) принимает обычную функцию `'a -> 'b`, а не `'a -> 'b option`.
...
`let*` «разворачивает» `option`: если `user.address = Some addr`, связывает `addr` с адресом и продолжает. Если `user.address = None` — вся функция сразу возвращает `None`, до `addr.zip` даже не доходит. Тип возвращаемого значения — `string option` (тот же, что у `addr.zip`). Никаких `match` вручную.
...
`let open ... in` ограничивает область видимости операторов одной функцией. Без него `let*` из `Option_syntax` и `let*` из `Result_syntax` конфликтовали бы — последнее `open` вытесняло бы предыдущее. Такая явная локализация — идиоматичный OCaml.
...
`combine_results` проходит по списку результатов и раскладывает их в две стопки: успешные значения и ошибки. Если ошибок нет — возвращает `Ok` со всеми значениями (в исходном порядке, поэтому `List.rev`). Если хоть одна ошибка — возвращает `Error` со всеми накопленными ошибками. В отличие от `let*`, который останавливается на первой ошибке, эта функция всегда обходит весь список.
...
Обёртка `try ... with` — стандартный способ «перехватить» исключение и превратить его в `result`. Это полезно при работе с кодом, который нельзя изменить (сторонние библиотеки, stdlib-функции вроде `int_of_string`).
...
`unwrap` — аварийное извлечение значения: если `Error`, бросает исключение. Используется там, где ошибка «не должна произойти» (например, в тестах). `try_result` — обратная операция: оборачивает произвольный вызов в `result`, перехватывая любые исключения. `Printexc.to_string` превращает исключение в читаемую строку.
...
`validate_address` запускает `validate_all` на каждом поле отдельно, а затем конкатенирует списки ошибок через `@`. Если все поля прошли — конструирует запись `{ street; city; state }`. Это ручная реализация того же накопления ошибок, что и `and*`, но без специального оператора.
...
Все четыре варианта ошибок из двух разных модулей обрабатываются в одном `match` — без общего типа и без `map_error`. Полиморфные варианты «открыты»: функция принимает `[> ...]`, то есть «хотя бы эти варианты», а конкретное множество определяет компилятор из кода `process_input`.
...
Синтаксис `let* ... and* ... and* ... in` — одна связанная конструкция. Все три валидации запускаются независимо, их ошибки накапливаются оператором `and*`. Только если все три вернули `Ok` — создаётся запись `{ name; age; email }`. Обратите внимание: `and*` — не ключевое слово языка, а обычный оператор, который мы определяем сами.
...
let outcome_ok ?(warnings = []) result =
  { result = Some result; errors = warnings }

let outcome_fail warnings =
  { result = None; errors = warnings }
```

`?(warnings = [])` — необязательный параметр с дефолтом. При вызове `outcome_ok password` предупреждений не будет; при `outcome_ok ~warnings:[...] password` — будут. Параметр называется `warnings`, но в записи он кладётся в поле `errors`.
...
Три случая в примере: `"abc"` — слишком короткий, операция провалилась (`result = None`); `"abcdef"` — принят, но с двумя предупреждениями (`result = Some ...`, `errors` непуст); `"myStr0ngPa55word"` — принят без замечаний. Отличие от `result`: промежуточный вариант (успех + предупреждения) невозможно выразить через `Ok`/`Error`.
...
- `option` и `result` — основные инструменты обработки ошибок.
- Let-операторы (`let*`, `let+`) — синтаксический сахар для цепочек вычислений.
- Накопление ошибок — паттерн для валидации нескольких полей одновременно.
- Четыре подхода к ошибкам: исключения, строки, кастомные типы, полиморфные варианты.
- Полиморфные варианты дают composable, различимые и exhaustive ошибки без boilerplate.
- `and*` — параллельный сбор ошибок вместо остановки на первой.
- Outcome type — для предупреждений наряду с результатом.

Соглашения по обработке ошибок и pattern matching см. в [Приложении «Конвенции OCaml»](appendix_d.md).

Следующая глава — мутабельное состояние и прямые эффекты: ввод-вывод, файлы и ссылки `ref`.
