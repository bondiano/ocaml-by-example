...
Эта бонусная глава посвящена **рекурсивным схемам** (recursion schemes) --- мощной технике обобщения рекурсивных вычислений. Если вы когда-нибудь замечали, что пишете один и тот же рекурсивный паттерн для списков, деревьев и AST, то рекурсивные схемы --- это способ абстрагировать этот паттерн раз и навсегда:
...
Код этой главы находится в `exercises/chapter22`. Соберите проект:

```text
$ cd exercises/chapter22
$ dune build
```
...
- Увидели **повторяющийся паттерн** рекурсивных вычислений в списках, деревьях, выражениях --- и осознали потребность в обобщении.
- Отделили **форму** данных от **рекурсии** через fix-point типы: `type 'a list_f = Nil | Cons of int * 'a` описывает один уровень, `type fix_list = Fix_list of fix_list list_f` завязывает рекурсию.
- Реализовали **четыре рекурсивные схемы**:
  - **Catamorphism** (обобщённый fold) --- сворачивает структуру снизу вверх по алгебре.
  - **Anamorphism** (обобщённый unfold) --- строит структуру сверху вниз по коалгебре.
  - **Hylomorphism** --- unfold + fold без промежуточной структуры.
  - **Paramorphism** --- fold с доступом к исходным подструктурам.
- Обобщили все схемы через **модульный функтор** `MakeSchemes`, написав рекурсию ровно один раз.
- Сравнили подход OCaml (модули) с подходом Haskell (классы типов и HKT).
- Применили рекурсивные схемы к **практической задаче** --- обработке JSON: pretty-print, вычисление глубины, генерация и трансформация.

Рекурсивные схемы --- это не просто академическое упражнение. Они дают **общий словарь** для описания рекурсивных вычислений: вместо «рекурсивная функция, которая обходит дерево и собирает результат» можно сказать «катаморфизм с такой-то алгеброй». Этот словарь помогает думать о структуре вычислений и находить новые комбинации.
