...
В этой главе разбираем **метапрограммирование** в OCaml — генерацию кода на этапе компиляции:

- **Метапрограммирование** — три подхода: Lisp, Rust, OCaml.
...
Заметьте, что сгенерированный `show_direction` добавляет **префикс модуля** (`Direction.North`, а не просто `North`). Это стандартное поведение `ppx_deriving.show` — оно помогает однозначно идентифицировать конструктор в контексте большой программы с несколькими модулями.
...
Для записей `show_person` генерирует строку с именами всех полей в синтаксисе OCaml. Обратные слеши перед кавычками внутри — это экранирование строки для вывода в utop; реальная строка содержит обычные кавычки.
...
- Разобрали конвейер компиляции OCaml: ppx работает после парсинга, но до типизации.
- Изучили два вида ppx: extension points (`[%name ...]`) и derivers (`[@@deriving ...]`).
- Применили ppx_deriving: `show` для строкового представления, `eq` для равенства, `ord` для сравнения.
- Научились смотреть сгенерированный код через `dune describe pp`.
- Рассмотрели архитектуру ppx-деривера на ppxlib.
- Сравнили ppx с Template Haskell и GHC Generics.

PPX-трансформации работают с типизированным AST — сгенерированный код синтаксически корректен по построению. На практике `[@@deriving ...]` покрывает большинство потребностей: от отладочного вывода до JSON-сериализации.

В следующей главе: оптимизации — как сделать OCaml-код быстрым.
