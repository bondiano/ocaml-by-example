...
В этой главе разбираем внутреннее устройство OCaml и техники оптимизации:
...
`Obj.repr` переводит любое значение OCaml в низкоуровневый тип `Obj.t`, а `Obj.is_int` проверяет, установлен ли тег-бит. Все четыре значения — `int`, `bool`, `unit`, `char` — являются immediate и возвращают `true`. Это подтверждает, что они не порождают аллокации в куче.
...
Все четыре значения возвращают `false` — они являются указателями на блоки в куче. Даже `Some 42` хранит `int` как immediate внутри блока, но сам конструктор `Some` требует аллокации.
...
Кортеж `(1, 2, 3)` имеет тег `0` (обычный блок) и размер `3` — три поля. Число `3.14` имеет тег `253` (Double), а строка `"hello"` — тег `252` (String). Зная эту таблицу, можно в низкоуровневом коде различать типы значений без информации от компилятора.
...
Тег `253` (Double) означает, что `Meters_u 1.0` в памяти неотличим от `1.0 : float` — никакого промежуточного блока нет. Для сравнения: `Meters 1.0` без аннотации имел бы тег `0` (обычный блок с одним полем-float).
...
Здесь `user` и `post` — **абстрактные типы-маркеры** (phantom types): они нигде не определены как конкретные типы, но участвуют в системе типов. `user id` и `post id` — разные типы с точки зрения компилятора, поэтому нельзя перепутать идентификатор пользователя с идентификатором поста. В рантайме же `user id` — это просто `int` (благодаря `[@@unboxed]`), никакой дополнительной памяти не тратится.
...
`[@@immediate]` — это **утверждение** компилятору: «я гарантирую, что этот тип помещается в машинное слово». Компилятор это проверяет — если какой-то конструктор несёт данные, он не может быть immediate, и компиляция завершится ошибкой. Это делает аннотацию безопасной: ошибочное использование ловится на этапе компиляции, а не в рантайме.
...
При специализации Flambda создаёт отдельную копию `map` для каждого конкретного `f`, устраняя накладные расходы на косвенный вызов через замыкание. Это особенно эффективно для часто вызываемых полиморфных функций — ценой небольшого увеличения размера кода.
...
Здесь используются два приёма: явная аннотация типа `float array` позволяет компилятору применить оптимизацию «flat float array» (массив хранит `float` напрямую без boxing), а `Array.unsafe_get` пропускает проверку границ индекса для максимальной скорости — применять только когда корректность индекса доказана.
...
`minor_words` и `major_words` — кумулятивные счётчики всего аллоцированного с начала программы (в словах, 8 байт каждое). Их разница между двумя точками замера показывает, сколько было аллоцировано в промежутке. `live_words` — снимок живых объектов прямо сейчас, `heap_words` — текущий размер кучи (включая свободные блоки).
...
- Разобрали представление значений в памяти: immediate-значения (int, bool, char, unit) хранятся в машинном слове, boxed-значения (float, строки, записи, кортежи) — в блоках в куче.
- Изучили структуру блока: заголовочное слово с тегом, размером и битами для GC.
- Применили `[@@unboxed]` для устранения обёрток и `[@@immediate]` для гарантии immediate-представления.
- Реализовали hash-consing с нуля — разделение памяти между структурно равными значениями с `O(1)` сравнением.
- Разобрали Flambda и его аннотации для управления инлайнингом и специализацией.
- Построили hash-consed AST и измерили разницу в потреблении памяти через `Gc.stat`.

Рантайм OCaml достаточно быстр по умолчанию. Для числовых вычислений, компиляторов и символьных систем описанные техники дают существенный прирост производительности.
