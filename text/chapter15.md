...
Библиотека **Cmdliner** для CLI-приложений на OCaml. Декларативное определение аргументов, опций и подкоманд. Интерактивный ввод через `read_line`.

Проект главы — **Brain Games**: 5 математических мини-игр, объединённых в одно CLI-приложение с подкомандами. Архитектура строится на двух типах: `round` (один раунд) и `game` (стратегия генерации вопросов).
...
Cmdliner автоматически генерирует справку из полей `~doc` и `~docv`. Описания команд в `--help` — это те самые строки `game.description`, переданные в `make_cmd` через `~doc:game.description`. Команды отсортированы алфавитно.
...
Конвертер `difficulty_conv` превращает строку `"easy"` в число `3` (количество раундов). При некорректном вводе `Error (`Msg ...)` заставляет Cmdliner вывести сообщение об ошибке и завершить программу с кодом 124, не вызывая основную функцию.
...
Функция `run` возвращает `int` — код выхода. `Cmd.eval cmd` вызывает `run` с распарсенными аргументами и возвращает её результат. `exit` передаёт этот код операционной системе. Такой паттерн позволяет скриптам и CI-системам определять успешность запуска по коду выхода.
...
5. **(Среднее)** Реализуйте команду `todo_add_cmd` для CLI todo-менеджера.

    ```ocaml
    val todo_add_cmd : unit Cmdliner.Cmd.t
    ```

    Команда должна принимать текст задачи как позиционный аргумент и добавлять её в список.

    *Подсказка:* используйте `Arg.(required & pos 0 (some string) None)` для позиционного аргумента и `Chapter15.Cli_tools.add_todo` для добавления задачи.

6. **(Среднее)** Реализуйте команду `todo_list_cmd` для вывода всех задач.

    ```ocaml
    val todo_list_cmd : unit Cmdliner.Cmd.t
    ```

    Команда не принимает аргументов и выводит список всех задач.

    *Подсказка:* используйте `Term.(const list_todos $ const ())` и `Chapter15.Cli_tools.list_todos`.

7. **(Среднее)** Реализуйте команду `calculator_cmd` — простой калькулятор.

    ```ocaml
    val calculator_cmd : unit Cmdliner.Cmd.t
    ```

    Команда принимает два числа (позиционные аргументы) и операцию через флаг `--op` (add, sub, mul, div).

    Пример: `calculator 5 3 --op add` выводит `8`.

    *Подсказка:* используйте `Arg.pos` для чисел и `Arg.opt` для операции.

8. **(Среднее)** Реализуйте функцию `grep` — поиск строк с паттерном.

    ```ocaml
    val grep : case_insensitive:bool -> string -> string list -> string list
    ```

    Функция возвращает список строк, содержащих паттерн. Флаг `case_insensitive` включает поиск без учёта регистра.

    *Подсказка:* используйте `String.lowercase_ascii` для нормализации и рекурсивную функцию для проверки вхождения подстроки.

## Заключение

Cmdliner работает через три понятия: `Arg` (описание аргумента), `Term` (связка функции с аргументами), `Cmd` (команда с именем и документацией). Аппликативный стиль `Term.(const f $ a $ b)` обеспечивает статическую типобезопасность.

Brain Games: brain-even, brain-calc, brain-gcd, brain-progression, brain-prime — пять подкоманд, построенных на общей архитектуре `round` и `game`.
