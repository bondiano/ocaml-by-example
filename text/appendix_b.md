...
**Lwt** — библиотека конкурентности на основе промисов. Разберём её и сравним с Eio из главы 12:
...
- Промисы `'a Lwt.t` — основа конкурентности в Lwt.
- Монадический стиль: `>>=`, `>|=`, `let*`, `let+`, `and*`.
- Конкурентные примитивы: `Lwt.both`, `Lwt.all`, `Lwt.pick`, `Lwt.choose`.
- Асинхронный I/O: `Lwt_io`, `Lwt_unix`, `Lwt_stream`.
- Сравнение Eio и Lwt: сильные стороны и области применения каждой.

Lwt — зрелая библиотека, на которой построена большая часть OCaml-экосистемы: Dream, Cohttp и другие. Dream рассматривается в главе 18 и использует Lwt под капотом.
