...
6. **(Среднее)** Реализуйте функцию `rate_limit`, которая применяет функцию к списку элементов с задержкой между вызовами.

    ```ocaml
    val rate_limit : clock:Eio.Time.clock -> ('a -> 'b) -> 'a list -> float -> 'b list
    ```

    Принимает функцию, список аргументов и задержку (в секундах) между вызовами. Возвращает список результатов.

    *Подсказка:* используйте рекурсию и `Eio.Time.sleep` для задержки между вызовами.

7. **(Сложное)** Реализуйте функцию `worker_pool`, которая создаёт пул воркеров фиксированного размера для обработки списка задач.

    ```ocaml
    val worker_pool : int -> (unit -> 'a) list -> 'a list
    ```

    Принимает количество воркеров и список задач, возвращает список результатов.

    *Подсказка:* используйте `Eio.Stream` для распределения задач и `Eio.Switch.run` с `Fiber.fork` для создания воркеров. Каждый воркер читает задачи из stream, выполняет их и отправляет результаты в другой stream.

8. **(Среднее)** Реализуйте функцию `parallel_process`, которая обрабатывает список «файлов» конкурентно и возвращает сумму результатов.

    ```ocaml
    val parallel_process : (string -> int) -> string list -> int
    ```

    Принимает функцию обработки (например, подсчёт длины строки) и список строк (имитация имён файлов).

    *Подсказка:* используйте `Eio.Fiber.List.map` для конкурентной обработки и `List.fold_left` для суммирования.
