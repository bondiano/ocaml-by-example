# Проектирование через типы (Часть 1)
...
Глава разделена на две части. В первой части мы изучим фундаментальные паттерны:
...
Во второй части мы рассмотрим паттерн **Parse, Don't Validate** и применим все техники в комплексном проекте — типобезопасной платёжной системе.
...
В этой части главы мы изучили два ключевых паттерна проектирования через типы:

- **Smart Constructors** — ограничение конструирования значений через валидацию. Абстрактные типы и приватные типы гарантируют, что некорректные значения (отрицательные суммы, невалидные email) не могут быть созданы.
- **Make Illegal States Unrepresentable** — кодирование допустимых состояний в самих типах. Вместо булевых флагов используются отдельные типы для каждого состояния, что делает невозможными состояния компиляции.
- **Phantom Types** — типовые параметры-метки для статической проверки протоколов без изменения представления данных.

Эти паттерны позволяют **сдвинуть проверки с рантайма на этап компиляции** — система типов OCaml гарантирует корректность ещё до запуска программы.

В следующей части главы мы рассмотрим паттерн **Parse, Don't Validate** и применим все изученные техники в комплексном проекте — типобезопасной платёжной системе.
