...
Если вы пишете на Python, TypeScript или Go, вам знакома ситуация: код работает, тесты проходят, но в продакшене вылетает `TypeError: Cannot read properties of undefined` или `KeyError`. Вы добавляете проверки, пишете больше тестов, подключаете mypy или strict-режим TypeScript --- но ошибки всё равно просачиваются.

OCaml решает эту проблему фундаментально:
