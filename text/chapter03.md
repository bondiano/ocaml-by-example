...
В этой главе мы познакомимся с двумя строительными блоками программ на OCaml: функциями и записями (records). Мы создадим простое приложение -- адресную книгу -- и на его примере разберём:
...
Код этой главы находится в `exercises/chapter03`. Основной модуль -- `lib/address_book.ml`. Соберите проект:
...
Здесь `{ street; city; state }` -- образец, который извлекает поля в одноимённые переменные. Это компактнее, чем обращение через точку.
...
Символ `_` означает «остальные поля нас не интересуют».
...
val addr2 : address = {street = "ул. Пушкина, 10"; city = "Санкт-Петербург"; state = "Санкт-Петербург"}

# addr.city;;
- : string = "Москва"
```

Выражение `{ addr with city = "Санкт-Петербург" }` создаёт копию `addr`, в которой поле `city` заменено на новое значение. Исходный `addr` не изменяется.
...
Здесь `( * ) 2` -- частичное применение оператора умножения: функция, которая умножает свой аргумент на 2.
...
Он передаёт значение слева как аргумент функции справа. Сравните:
...
Функция `List.find_opt` ищет первый элемент, удовлетворяющий предикату, и возвращает `option`. Это безопаснее, чем `List.find`, которая бросает исключение `Not_found`.
...
Обе записи эквивалентны. Выбор стиля -- дело вкуса, но конвейер чаще используется при цепочке нескольких операций.
...
- Определили типы данных с записями (`type`).
- Научились создавать записи, обращаться к полям и обновлять их (`{ r with ... }`).
- Разобрали каррирование и частичное применение.
- Познакомились с именованными аргументами (`~label`) и опциональными аргументами (`?opt`).
- Освоили оператор конвейера `|>` -- идиоматическую замену `($)` и `(.)` из Haskell.
- Познакомились с типом `option` для безопасной обработки отсутствующих значений.

В следующей главе мы подробно разберём алгебраические типы данных и сопоставление с образцом.
