...
Код этой главы находится в `exercises/chapter04`. Основной модуль --- `lib/shapes.ml`. Соберите проект:
...
Модуль `Result` предоставляет функции `Result.map`, `Result.bind`, `Result.is_ok`, `Result.is_error` --- аналогично модулю `Option`. Подробно мы рассмотрим `result` в главе 7.
